...
from ltae import PhenologyLTAE2d
...
class GlobalLocalAttention(nn.Module):
    ...
    def pad(self, x, ps):
        _, _, H, W = x.size()
        if W % ps != 0:
            x = F.pad(x, (0, ps - W % ps, 0, 0), mode='replicate')
        if H % ps != 0:
            x = F.pad(x, (0, 0, 0, ps - H % ps), mode='replicate')
        return x
...
class TemporalPhenologyFusion(nn.Module):
    def __init__(
        self,
        in_channels,
        time_steps,
        ...
        ltae_dropout=0.1,
        attn_dropout=0.1,
        phenology_prior_mode="learned",
        fusion_scale=1.0,
    ):
        super().__init__()
        if bands_per_step is None:
            if in_channels % time_steps != 0:
                raise ValueError(
                    "in_channels must be divisible by time_steps when bands_per_step is None."
                )
            bands_per_step = in_channels // time_steps
        self.time_steps = time_steps
        self.bands_per_step = bands_per_step
        self.fusion_scale = float(fusion_scale)
        self.ltae = PhenologyLTAE2d(
            in_channels=bands_per_step,
            n_head=ltae_heads,
            d_k=ltae_d_k,
            d_model=ltae_d_model,
            mlp=[ltae_d_model, in_channels],
            dropout=ltae_dropout,
            T=time_steps,
            attn_dropout=attn_dropout,
            phenology_prior_mode=phenology_prior_mode,
        )
    ...
    def forward(self, x, batch_positions=None, quality_score=None, pad_mask=None):
        b, c, h, w = x.shape
        expected = self.time_steps * self.bands_per_step
        if c != expected:
            raise ValueError(
                f"Expected {expected} channels, got {c}."
            )
        source = x
        if self.fusion_scale < 1.0:
            # 区域微调样本较少且 patch 较大时，先在低分辨率上计算 P-LTAE，
            # 再插值回原尺寸，显著降低逐像元时序注意力的显存和耗时。
            scaled_h = max(1, int(round(h * self.fusion_scale)))
            scaled_w = max(1, int(round(w * self.fusion_scale)))
        ...
        x_seq = source.view(b, self.time_steps, self.bands_per_step, sh, sw)
        ltae_out = self.ltae(
            x_seq,
        ...
        if ltae_out.shape[-2:] != (h, w):
            ltae_out = F.interpolate(
                ltae_out,
                size=(h, w),
                mode="bilinear",
                align_corners=False,
            )
        gated = torch.tanh(self.gate) * ltae_out
        return x + gated
...
class RS3Mamba(nn.Module):
    def __init__(self,
        ...
        self.use_phenology_fusion = use_phenology_fusion
        if self.use_phenology_fusion:
            self.temporal_fusion = TemporalPhenologyFusion(
                in_channels=in_channels,
                time_steps=time_steps,
                bands_per_step=bands_per_step,
                ltae_d_model=ltae_d_model,
                ltae_heads=ltae_heads,
                ltae_d_k=ltae_d_k,
                ltae_dropout=ltae_dropout,
                attn_dropout=attn_dropout,
                phenology_prior_mode=phenology_prior_mode,
                fusion_scale=phenology_fusion_scale,
            )
